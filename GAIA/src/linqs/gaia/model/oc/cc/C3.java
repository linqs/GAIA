 ...
 * <p>
 * As with {@link linqs.gaia.model.oc.cc.ICA}, the relational features are implemented through
 * defining and specifying {@link linqs.gaia.feature.DerivedFeature} for the items being predicted.
 * <p>
 * To improve runtime performance and to aid in analysis, a number of functions called at various
 * points in the learning and inference can be overridden (i.e., , {@link #preLearning(List)},
 * {@link #preRelationalLearning(List)}, {@link #postLearning(List)}, {@link #prePredicting(List)},
 * {@link #preIteration(List, int)}, {@link #postIteration(List, int)},
 * {@link #postPredicting(List)}).
 * 
 * <p>
 * Required Parameters:
 * <UL>
 * <LI>tids-Comma delimited list of task ids with a 1-1 correspondence to the list of schema and
 * feature IDs specified during the learn methods. The task IDs are used to specify the classifiers
 * and features used for each task. For example, the first item in this delimited list is used to
 * specify the classifiers and features used to predict the value specified by the first schema and
 * feature ID specified.
 * <LI>&lt;tid&gt;-nonrelvbc-Vector based classifier ({@link VBClassifier}) to use on non-relational
 * features, for the task corresponding to the task id &lt;tid&gt;, instantiated using in
 * {@link Dynamic#forConfigurableName}.
 * <LI>&lt;tid&gt;-relvbc-Vector based classifier ({@link VBClassifier}) to use on relational
 * features, for the task corresponding to the task id &lt;tid&gt;, instantiated using in
 * {@link Dynamic#forConfigurableName}.
 * </UL>
 * <p>
 * Optional Parameters:
 * <UL>
 * <LI>&lt;tid&gt;-includenonrelfeatures-Non-relational/local features, in the schema of the target
 * feature, used during bootstrapping. If not defined, all features NOT of type Derived are used.
 * The parameters are treated as a comma delimited list of feature ids and/or regex pattern for
 * feature IDs. Format defined in {@link FeatureUtils#parseFeatureList(String, List)}.
 * <LI>&lt;tid&gt;-excludenonrelfeatures-Non-relational/local features excluded during
 * bootstrapping. Format defined in {@link FeatureUtils#parseFeatureList(String, List)}.
 * <LI>&lt;tid&gt;-includerelfeatures-Relational features used after bootstrapping. If not defined,
 * all features of type Derived are used. Format defined in
 * {@link FeatureUtils#parseFeatureList(String, List)}. Note: Relational classifiers are run using
 * the union of the defined non-relational and relational features.
 * <LI>&lt;tid&gt;-excluderelfeatures-Relational features to exclude after bootstrapping. Format
 * defined in {@link FeatureUtils#parseFeatureList(String, List)}. Note: Relational classifiers are
 * run using the union of the defined non-relational and relational features.
 * 
 * <LI>numiterations-Number of iterations to do. Set to 0 to only use the non-relational classifier.
 * Default is 10.
 * <LI>bootstraplearning-If "yes", apply local classifier to unlabeled instances prior to training.
 * This bootstraps those instances to allow for relational features in a semi-supervised
 * classification setting. Training is still only performed using the annotated instances as the
 * bootstrap values are only used in the relational features. Default is "no".
 * <LI>ordertype-Class of {@link ItemOrder} used to order instances within each iteration. Default
 * is {@link linqs.gaia.model.util.itemorder.RandomOrder}.
 * <LI>batchupdate-If set to "yes", only update the labels at the end of each iteration such that
 * the value of relational features are based on the values of the previous iteration. If set to
 * "no", set labels within the iteration such that the value of relational features are based on the
 * most recent value, even with the same iteration, of the labels. Default is "yes".
 * 
 * <LI>permanentcommitkpct-If "yes", at each iteration, only updates the most confident (in terms of
 * probability of the most likely value) items to their predicted values at the end of each
 * iteration. Once set in an iteration, the values of these items is not updated in later
 * iterations.
 * <LI>kpercent-A value between 0 (exclusive) and 1 (inclusive) which specifies the proportion of
 * labels to commit, for each task, at the end of each iteration. The items labeled are chosen as
 * the top K most confident (in terms of probability of the most likely value) items. If
 * permanentcommitkpct is "yes", the K is computed as K=(kpercent * number of items for each task).
 * If permanentcommitkpct is "no", the K is computed relative to the current iteration
 * (kpercent*currentiteration*number of items). If kpercent<1, the batchupdate parameter is
 * automatically set to "yes".
 * 
 * <LI>checkoscillation-If "yes", check for an oscillation by storing the predicted values at every
 * iteration and comparing (including the probability distribution) the predicted values of the
 * current iteration with the predicted values of a previous iteration. If the predicted values of
 * the current iteration exactly match the predicted values of a previous iteration, an oscillation
 * or convergence has been reached and we can stop at this iteration. Default is "no".
 * <LI>ignoreprobs-If "yes", ignore the probabilities when checking for oscillation. Default is
 * "no".
 * <LI>checkconvergence-If "yes", check for convergence by looking at the number of values changed
 * per iteration and stopping when there are no changes made, with the predicted labels (excluding
 * the probability distribution) in a particular iteration. Default is "no".
 * <LI>minnumiterations-If specified, do not stop executing either by detecting convergence or
 * oscillation unless this specified minimum number of oscillations has already been reached.
 * 
 * <LI>performstochastic-If "yes", instead of deterministically choosing the most likely label at
 * every iteration, do a stochastic sampling instead and select the label from the distribution
 * returned by the classifier. Default is "no".
 * <LI>performstochastictemp-If "yes", instead of deterministically choosing the most likely label
 * at every iteration, with probability (# current iteration/maximum number of iteration) do a
 * stochastic sampling instead and select the label from the distribution returned by the
 * classifier. Default is "no".
 * <LI>stochastictempiter-If specified, instead of using the maximum number of iterations as the
 * denominator of the probability defined for performstochastictemp, use the value as the
 * denominator instead.
 * 
 * <LI>performsampling-If "yes", instead of performing the prediction by selecting the most likely
 * label at every iteration, a sampling procedure is performed where the current label is drawn from
 * the current probability distribution. The sampled label is stored for each item and the final
 * distribution is computed by looking at the number of times a given label was sampled for each
 * item. Default is no.
 * <LI>seed-The seed to use for the random number generator used when performing sampling. Default
 * is 0.
 * <LI>burnin-The number of iterations to ignore, from the beginning, as burn-in for the sampling.
 * Note: The number of samples is (numiterations-burnin). Default is 0.
 * 
 * <LI>performstacking-If "yes", instead of learning a single relational classifier to apply at
 * every iteration, perform stacked learning to learn a different classifier at each iteration.
 * Default is "no".
 * <LI>predlabelfids-When performing stacked learning of the relational features, you need to learn
 * from features computed over the predicted value, instead of the true value. To accomplish that,
 * you need to specify a temporary feature to store that predicted value for use in the features.
 * This parameter is a comma delimited string where the entries correspond to each of the target
 * feature ids specified during learning and where where the temporary feature, correspoding to each
 * target feature id, is used during the stacked learning. Only applicable if performstacking=yes.
 * <LI>&lt;tid&gt;-samplerclass-For each specified task id (given by tids parameter), specify the
 * {@link DecorableSampler} class to use in sampling items of that task for the cross-validation in
 * stacked learning. Required if performstacking=yes.
 * <LI>unlabeledstackingonly-If "yes", run the variant of stacking where you learn a different set
 * of classifiers per iteration but only over the labeled instances. No cross validation is done but
 * prior to learning the classifier for the current iteration, apply the classifier from the
 * previous iteration. Only applicable if performstacking=yes. Default is "no".
 * <LI>emstacking-If "yes", run the variant of stacking where you learn a different set of
 * classifiers per iteration using both the labeled and unlabeled instances. Only applicable if
 * performstacking=yes. Default is "no".
 * <LI>saveperiterdir-Directory to use to save the predicted values of each item for each iteration.
 * A file named using the iteration number (i.e., 1.txt) is created for each iteration with two tab
 * delimited colums, the first containing a string representation of the object and the second a
 * string representation of the feature value. Only applicable if performstacking=yes.
 * <LI>checklearningconvergence-If "yes", check for convergence when learning. Once convergence is
 * detected, there is no need to learn any more levels. Default is "no".
 * 
 * <LI>printtrainderivetime-If "yes", print the time it takes to compute the features of the graphs
 * of the train items (computed using {@link GraphUtils#printDerivedTime(Graph)}. This is useful for
 * analyzing performance. Default is "no".
 * <LI>printtrainvalues-If "yes", print a summary of the features of the graphs of the train items
 * (computed using {@link GraphUtils#printFeatureValueOverview(Graph)}). This is useful for
 * analyzing performance. Default is "no".
 * <LI>printbootstrap-If "yes", print the predicted value of each item after bootstrapping. This is
 * useful for analyzing performance. Default is "no".
 * <LI>printchanges-If "yes", print the changes found in each iteration. This is useful for
 * analyzing performance. Default is "no".
 * <LI>printtestderivetime-If "yes", print the time it takes to compute the features of the graphs
 * of the test items (computed using {@link GraphUtils#printDerivedTime(Graph)}. This is useful for
 * analyzing performance. Default is "no".
 * <LI>printoscillation-If "yes", print the iteration that the oscillation was encountered, as well
 * as what previous iteration was the start of the oscillation. Default is "no".
 * <LI>printnumiterations-If "yes", print the number of iterations run during inference. Default is
 * "no".
...
public class C3 extends BaseConfigurable
    ...
    public void learn(List<Iterable<? extends Decorable>> trainitems,
        ...
        if (this.performstacking) {
            ...
            for (int l = 0; l < numiterations; l++) {
                Map<SimplePair<Decorable, Integer>, FeatureValue> batchvalues = new HashMap<SimplePair<Decorable, Integer>, FeatureValue>();
    ...
    /**
     * Return relational classifier for items in the current index for the specified iteration.
     * 
    ...
    /**
     * Method called prior to learning any classifiers. Can be overloaded to do any preprocessing or
     * analysis prior to learning the classifiers.
     * 
    ...
    /**
     * Method called prior to learning the relational classifiers. When doing stacked learning, this
     * method is also called prior to learning the classifier for the current iteration. Can be
     * overloaded to do any preprocessing or analysis prior to learning the relational classifiers.
     * 
     * @param trainitems Train items passed to learn methods.
     */
    ...
    /**
     * Method called after to learning the classifiers. Can be overloaded to do any postprocessing
     * or analysis after learning the classifiers.
     * 
     * @param trainitems Train items passed to learn methods.
     */
    ...
    /**
     * Method called prior to applying any prediction. Can be overloaded to do any preprocessing or
     * analysis before applying the classifiers.
     * 
    ...
    /**
     * Method called after applying any prediction. Can be overloaded to do any postprocessing or
     * analysis after applying the classifiers.
     * 
     * @param trainitems Test items passed to predict methods.
     */
    ...
    /**
     * Method called at the beginning of an iteration during inference. Can be overloaded to do any
     * preprocessing or analysis before applying the classifiers in the current iteration.
     * 
     * @param trainitems Test items passed to predict methods.
    ...
    /**
     * Method called at the end of an iteration during inference. Can be overloaded to do any
     * postprocessing or analysis after applying the classifiers in the current iteration.
     * 
     * @param trainitems Test items passed to predict methods.
     * @param iteration Number of the current iteration (starting at 0).
     */
