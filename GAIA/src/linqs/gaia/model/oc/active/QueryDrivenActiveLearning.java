...
/**
 * Interface for query driven active learning algorithms.
 * In query driven active learning, unlike the standard active learning algorithm,
 * we are in a transductive setting where we are only interested in optimizing
 * the learning for the specific queries.
 * 
 * @author namatag
 *
 */
public interface QueryDrivenActiveLearning extends Model {
	/**
	 ...
	 * of the query.
	 * 
	 * @param queries Query we wish to optimize the active learning for
	 * @param g Graph
	 * @param numqueries Number of queries to return
	 * @return Ordered list of items and features
	 */
	List<Query> getQueries(Iterable<Query> queries,
			Graph g,
			int numqueries);
	 ...
	 */
	List<Query> getQueries(Iterable<Query> queries,
			Iterable<? extends Decorable> testitems,
			int numqueries);
}
