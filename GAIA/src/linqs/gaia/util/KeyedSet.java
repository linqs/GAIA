...
/**
 * Store object in an Set keyed by a given key. Can be useful for hashing or just keeping objects in
...
public class KeyedSet<K, V> extends KeyedCollection<Set<V>, K, V> {
     ...
     * @param returnempty If true, return an empty set when getting a collection which has no items.
    ...
    public Set<V> getSet(K key) {
        ...
        if (returnempty) {
            ...
            return set == null ? null : new HashSet<V>(set);
        }
    }
